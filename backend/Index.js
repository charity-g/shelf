...
app.get("/products", (req, res) => {
    const products = Object.values(mockProductsDB).map(({ id, name, brand, category }) => ({
        id,
        name,
        brand,
        category,
...
app.get("/products/:id/similar", (req, res) => {
    ...
    const productIngredients = new Set(product.ingredients.map(i => i.toLowerCase()));
    ...
    const similarProducts = Object.values(mockProductsDB)
        ...
        .map(p => {
            ...
            const matchPercentage = Math.round(
                (sharedIngredients.length / Math.max(product.ingredients.length, p.ingredients.length)) * 100
            );
